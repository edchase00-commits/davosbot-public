...
def prompt_namespace(source: str, *, roast_source: str = "") -> dict:
    ...
    if roast_source:
        helper_nodes = [node for node in ast.parse(roast_source).body
                        if isinstance(node, (ast.FunctionDef, ast.Assign, ast.AnnAssign))]
        exec(compile(ast.Module(body=helper_nodes, type_ignores=[]), "synthetic-roast-intent", "exec"), namespace)
...
def _uses_roast_helper(source: str) -> bool:
    return any(isinstance(node, ast.ImportFrom) and node.module == "roast_intent"
...
def main(argv=None) -> int:
    ...
    before_roast_source = (_git_output("show", f"{baseline_ref}:davosbot/roast_intent.py")
                           if _uses_roast_helper(baseline) else "")
    after_roast_source = ((ROOT / "davosbot/roast_intent.py").read_text(encoding="utf-8")
                          if _uses_roast_helper(after_source) else "")
    variants = {
        "before": prompt_namespace(baseline, roast_source=before_roast_source),
        "after": prompt_namespace(after_source, roast_source=after_roast_source),
    }
    if args.after_only:
        variants.pop("before")
    ...
    args.output.write_text(json.dumps({"model": args.model, "model_digest": installed[args.model].get("digest"),
                                      ...
                                      "roast_helper_sha256": {
                                          "before": hashlib.sha256(before_roast_source.encode()).hexdigest() if before_roast_source else None,
                                          "after": hashlib.sha256(after_roast_source.encode()).hexdigest() if after_roast_source else None,
                                      },
