...
_BLOCKED_ALERT_STATE_FILENAME = "blocked_alert_state.json"
# Outcome states that mean the pipeline got past the blocked gates again.
_RECOVERED_OUTCOME_STATES = {"up_to_date", "deployed", "waiting", "dry_run"}


def _blocked_reminder_seconds() -> int:
    return max(0, _int_env("AUTO_DEPLOY_BLOCKED_REMINDER_SECONDS", "21600"))


def _blocked_state_path(config: DeployConfig) -> Path:
    return config.status_path.parent / _BLOCKED_ALERT_STATE_FILENAME


def _read_blocked_state(config: DeployConfig) -> dict[str, Any] | None:
    try:
        payload = json.loads(_blocked_state_path(config).read_text(encoding="utf-8"))
...
def _write_blocked_state(config: DeployConfig, detail: str) -> None:
    try:
        _write_json_atomically(
            _blocked_state_path(config),
            {
                "version": 1,
                "detail": detail,
        ...
        LOG.warning("could not persist blocked-deploy alert state")


def _clear_blocked_state(config: DeployConfig) -> dict[str, Any] | None:
    previous = _read_blocked_state(config)
    try:
        _blocked_state_path(config).unlink(missing_ok=True)
    except OSError:
        LOG.warning("could not clear blocked-deploy alert state")
    return previous


def _stateful_blocked_alert(
    config: DeployConfig,
    alert: Callable[[str, str, dict[str, Any] | None], bool],
) -> Callable[[str, str, dict[str, Any] | None], bool]:
    """Wrap the alert callable so auto_deploy_blocked fires on state changes.

    The watcher polls every couple of minutes, so the plain cooldown still
    pages the owner ~96 times a day for one stuck deploy. Instead: alert once
    on the transition into blocked (or when the block reason changes), then at
    most every AUTO_DEPLOY_BLOCKED_REMINDER_SECONDS (default 6h) while the
    same block persists. State is persisted next to status.json (a gitignored
    dir) so watcher restarts don't re-trigger the transition alert.
    """

    def _wrapped(
        event_type: str, message: str, metadata: dict[str, Any] | None = None
    ) -> bool:
        if event_type != "auto_deploy_blocked":
            return alert(event_type, message, metadata)
        try:
            previous = _read_blocked_state(config)
        except Exception:  # pragma: no cover - alert bookkeeping must never break deploys
            previous = None
        # Wall clock (not monotonic): the state must survive process restarts.
        now = time.time()
        transition = previous is None or previous.get("detail") != message
        detail = message
        if not transition:
            last_alerted = previous.get("alerted_at") or 0
            try:
                last_alerted = float(last_alerted)
            except (TypeError, ValueError):
                last_alerted = 0
            reminder_seconds = _blocked_reminder_seconds()
            if not reminder_seconds or now - last_alerted < reminder_seconds:
                return False
            message = f"still blocked: {message}"
        sent = alert(event_type, message, metadata)
        _write_blocked_state(config, detail)
        return sent

    return _wrapped


def _maybe_alert_recovered(
    config: DeployConfig,
    outcome_state: str,
    alert: Callable[[str, str, dict[str, Any] | None], bool] = _send_alert,
) -> bool:
    """Send one recovery notice after a blocked pipeline starts flowing again."""
    if outcome_state not in _RECOVERED_OUTCOME_STATES:
        return False
    previous = _clear_blocked_state(config)
    if previous is None:
        return False
    detail = previous.get("detail") or "deploy was blocked"
    return alert("auto_deploy_recovered", f"auto-deploy recovered: {detail}", None)
...
def deploy_once(
    ...
    *,
    ...
    # State-change alerting for blocked deploys (transition + 6h reminders).
    alert = _stateful_blocked_alert(config, alert)
...
def main() -> int:
    ...
    while True:
        ...
        _maybe_alert_recovered(config, outcome.state)
