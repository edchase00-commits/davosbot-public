...
import json
import uuid
from .action_grounding import ground_unexecuted_reply
...
class ToolTrace:
    ...
    def reply(self, model_text: str | None = None) -> str | None:
        if not self.receipts:
            text = (model_text.strip() or None) if model_text is not None else None
            return ground_unexecuted_reply(self.user_msg, text)
        if all(receipt.name in READ_ONLY_TOOLS for receipt in self.receipts) and model_text and model_text.strip():
            if is_terminal_lookup_placeholder(self.user_msg, model_text):
                # Public lookup output is already intended for this reply. Do
                # not newly expose file, chat, or other private read results.
                from .permissions import redact_secret
                sections = []
                public = [receipt for receipt in self.receipts
                          if receipt.name in {"web_search", "get_weather"} and not receipt.duplicate]
                for receipt in public[-3:]:
                    outcome = receipt.outcome
                    detail = redact_secret(outcome.text).strip()
                    detail = detail[:1200] + ("\n[remaining output omitted]" if len(detail) > 1200 else "")
                    label = receipt.name.replace("_", " ")
                    # Even confirmed means only its recorded verification
                    # scope, never that the requested factual answer is true.
                    section = (
                        f"{label}: reported result (status: {outcome.status}; "
                        f"scope: {outcome.verification_scope})"
                    )
                    section += f"\nReported: {detail}" if detail else "\nNo result text was returned."
                    if outcome.error:
                        section += "\nError: " + redact_secret(str(outcome.error))[:200]
                    sections.append(section)
                if len(public) > 3:
                    sections.append("[earlier public lookup output omitted]")
                return "\n\n".join(sections) if sections else NO_LOOKUP_RESULT
            readback_tools = frozenset(
                receipt.name
                for receipt in self.receipts
                if receipt.outcome.status == "confirmed"
            )
            return ground_unexecuted_reply(self.user_msg, model_text.strip(), readback_tools=readback_tools)
