...
def is_owner_in_chat(chat_identifier: str, owner_id: str) -> bool:
    if not isinstance(owner_id, str) or not isinstance(chat_identifier, str) or not chat_identifier:
        return False
    owner = normalize_handle(owner_id)
    if not re.fullmatch(r"\+[1-9]\d{6,14}|[^@\s]+@[^@\s]+\.[^@\s]+", owner):
        return False
    try:
        conn = _get_db()
        try:
            ...
            conn.close()
        # Messages can retain an older phone spelling or email casing. Use the
        # same identity normalization as sender authorization, within this chat.
        return any(isinstance(row[0], str) and normalize_handle(row[0]) == owner for row in rows)
    except Exception as e:
        logger.error("is_owner_in_chat error (%s)", type(e).__name__)
        return False
