...
_REQUEST_PREFIX_RE = re.compile(
    ...
    r"(?:show|send|give|tell|list|find|fetch|check|pull\s+up|look\s+up)(?:\s+(?:me|us))?"
    r"|(?:can|could)\s+i\s+(?:get|see|have)"
    r"|what(?:'s|\s+(?:is|are))?|which|any"
    r"|who(?:'s|\s+(?:is|are))(?:\s+fighting)?"
    r"|when(?:'s|\s+(?:is|are|do|does))"
    r"|what\s+time(?:\s+(?:is|are|do|does))?"
    r")\s+",
    re.IGNORECASE,
)
_CARD_QUERY_WORD_RE = re.compile(
    r"(?:the|a|any|full|main|co-main|early|prelims?|preliminaries|fights?|cards?|"
    r"lineup|schedule|odds|ufc|mma|events?|tonight(?:'s)?|tomorrow(?:'s)?|today(?:'s)?|"
    r"this|next|week|weekend|upcoming|on|in|for|and|or|at|start|starts|starting|"
    r"begin|begins|beginning|are|is|do|does|time|times|please|pls)",
    re.IGNORECASE,
)


def is_ufc_fight_card_request(text: str) -> bool:
    """Match a bounded card query, not an incidental mention inside a report.

    This shortcut returns the upcoming card without asking the model what the
    user meant. Only consume complete sports-query wording; code, quoted text,
    failure reports and broader discussions must retain their normal route.
    Numbered or named events need normal lookup, not the upcoming-card feed.
    """
    clean = (text or "").strip().replace("\u2019", "'").rstrip("?!.").strip()
    if not _CARD_TOPIC_RE.search(clean):
        return False
    query = _REQUEST_PREFIX_RE.sub("", clean, count=1)
    return bool(query) and all(_CARD_QUERY_WORD_RE.fullmatch(word) for word in query.split())
