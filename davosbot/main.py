...
from .brain import (
    get_response, detect_capability_gap, log_error,
...
from . import roast_intent as _roast_intent
...
from . import acute_safety as _acute_safety
from . import file_continuation as _file_continuation
from . import identity_status as _identity_status
from . import rate_limit_notice as _rate_limit_notice
from .request_intent import is_log_analysis_request
...
_LIVE_INFO_TOOL_RE = re.compile(
    ...
    r"|^\s*(?:will|is|was)\s+it\s+(?:be\s+|going\s+to\s+)?"
    r"(?:rain(?:ing)?|snow(?:ing)?|cold|hot|warm|windy|sunny|cloudy)\b"
    # Natural place questions need current locations/hours, even without "search".
    r"|^\s*(?:(?:please|pls)\s+)?(?:(?:find|show\s+me|what(?:'s|\s+is)|where(?:'s|\s+is))\s+)?"
    r"(?:the\s+)?(?:nearest|closest)\s+\S"
    r"|^\s*(?:where|what|which|is|are|can\s+you|could\s+you|recommend|suggest|find)\b.{0,80}"
    r"\b(?:eat|dine|breakfast|lunch|dinner|restaurants?|cafes?|coffee\s+shops?|bars?|hotels?|stores?)\b"
    r".{0,90}\b(?:near(?:by)?|in|at|around|open)\b"
    r"|^\s*(?:when|what\s+time)\b.{0,70}\b(?:play|games?|concert|show|event)\b"
    r".{0,50}\b(?:next|today|tonight|tomorrow|this\s+week(?:end)?)\b",
    re.IGNORECASE,
)
_OWNER_SIDE_EFFECT_TOOL_RE = re.compile(
    ...
    r"(?:save|export)\b.{0,80}\b(?:file|csv|tsv|xlsx?|txt|json|spreadsheet|sheet|workbook|md|markdown)\b"
...
def _is_roast_request(text: str) -> bool:
    return _roast_intent.is_roast_request(text)
...
def _market_fast_reply(sender: str, text: str, *, skip_web_search: bool = False, originating_chat_id: str = "") -> str | None:
    from . import cron_creation
    from .tools import _parse_cron_schedule_command

    if (
        _parse_cron_schedule_command(text) is not None
        or cron_creation.is_creation_request(text)
        or cron_creation.is_schedule_discussion(text)
        or cron_creation.is_pending_followup(sender, originating_chat_id or sender, text)
    ):
        return None
...
_SELF_STATUS_QUESTION_RE = re.compile(
    ...
    r"|what\s+(?:can|do)\s+(?:you|davos|davosbot)\s+do(?=(?:\s+for\s+me)?[.!?]*\s*$)"
...
def _handle_screenshot_issue_log(
    ...
    if is_log_analysis_request(text):
        return None
...
def _handle_deterministic_reminder_schedule(text: str, originating_chat_id: str) -> str | None:
    from .reminder_parser import ReminderParseError
    try:
        parsed = parse_deterministic_reminder(text or "")
    except ReminderParseError as exc:
        return f"I didn't save a reminder. {exc}"
    if parsed is None:
        return None
...
def _parse_reminder_cancel_positions(text: str) -> list[int]:
    from .reminder_parser import parse_reminder_cancel_positions
    return parse_reminder_cancel_positions(text)
...
def _handle_deterministic_reminder_cancel(text: str, originating_chat_id: str) -> str | None:
    ...
    try:
        ...
        return f"I didn't cancel any reminders. {exc}"
    if not positions:
        return None
...
def _native_command_has_safe_history(text: str) -> bool:
    """Only reviewed read-only command forms may add their result to context."""
    from .commands import _is_persona_catalog_request, _looks_like_cleanup_status_request
    from .status_intent import is_billing_status_request
    from .dashboard_links import is_dashboard_link_request
    from .ufc import is_ufc_fight_card_request

    clean = re.sub(r"\s+", " ", (text or "").strip()).lower()
    if clean in {
        "help", "capability", "capabilities", "mypermissions", "status", "uptime",
        "billing", "api", "apis", "api status", "apis status", "tool status",
        "tools status", "tools", "fantasy", "cron help", "crons help", "jobs help",
    }:
        return True
    if re.fullmatch(
        r"models?(?:\s+(?:status|routing|routes|help|options?|models|list|intensity|ladder|tiers|modes?))?",
        clean,
    ):
        return True
    return (
        is_billing_status_request(clean)
        or _looks_like_cleanup_status_request(clean)
        or is_dashboard_link_request(clean)
        or is_ufc_fight_card_request(clean)
        # The exact DM command includes file-validation diagnostics, not just a catalog.
        or (clean != "personalities" and _is_persona_catalog_request(clean))
    )


def _send_native_command_reply(
    sender: str, text: str, reply: str, *, recipient: str | None = None,
    is_group: bool = False, known_read_only: bool = False,
) -> None:
    """Record a safe delivered result without retrying sends on history failure."""
    target = recipient or sender
    sent = send_message(target, reply, **({"is_group": True} if is_group else {}))
    if not sent:
        return
    try:
        if not (known_read_only or _native_command_has_safe_history(text)):
            return
        # Authentication/private-send text must never become ordinary model context.
        safe_text = redact_secret(redact_private_send_text_for_log(text))
        credential_text = re.search(
            r"\b(?:passwords?|passphrase|passcode|pw|otp|verification\s+code|auth(?:orization)?\s+code|api[_ -]?key|access[_ -]?token)\b",
            text, re.IGNORECASE,
        )
        if credential_text or check_admin_password(text) or safe_text != text:
            return
        from .text_safety import normalize_bot_text
        safe_reply = normalize_bot_text(redact_secret(redact_private_send_text_for_log(reply)))
        save_turn(target, "user", f"{sender}: {safe_text}" if is_group else safe_text)
        save_turn(target, "assistant", safe_reply)
    except Exception as exc:
        logger.warning("Delivered native command history failed (%s)", type(exc).__name__)


def _clear_superseded_cron_drafts(sender: str, text: str, recipient: str, *, is_group: bool = False, has_image: bool = False) -> None:
    """An explicit different owner workflow supersedes pending cron setup only."""
    if has_image or not is_owner(sender):
        return
    if not (_file_continuation.initial_request(text) or (not is_group and detect_reminder_edit_intent(text))):
        return
    from . import cron_creation, research_cron
    cron_creation.clear_draft(sender, recipient)
    research_cron.clear_draft(sender, recipient)


def _capture_cron_reply(producer, *args, **kwargs):
    from .cron_clarifications import capture_call
    return capture_call(producer, *args, **kwargs)


def _send_cron_reply(sender: str, text: str, recipient: str, reply: str, questions,
                     *, is_group: bool = False, record_history: bool = True) -> None:
    """One native receipt send; failed questions cannot leave hidden drafts."""
    from .cron_clarifications import discard_unseen
    try:
        sent = send_message(recipient, reply, **({"is_group": True} if is_group else {}))
    except Exception:
        sent = False
        logger.warning("Native cron reply send failed; not retrying.")
    if not sent:
        discard_unseen(questions)
    if record_history:
        try:
            incoming = f"{sender}: {redact_secret(text)}" if is_group else redact_secret(text)
            save_turn(recipient, "user", incoming)
            if sent:
                save_turn(recipient, "assistant", reply)
        except Exception:
            logger.warning("Native cron reply history failed; not resending.")


def _handle_file_continuation(sender: str, text: str, recipient: str, *, is_group: bool = False, has_image: bool = False) -> bool:
    """Continue one owner save after private-send/access/safety checks."""
    if has_image or not is_owner(sender):
        return False
    selected = _file_continuation.consume(sender, recipient, text)
    if selected is None:
        return False
    reply = selected
    if isinstance(selected, _file_continuation.Selection):
        attempts = []
        system = build_system_prompt(
            persona=get_persona(recipient if is_group else "dm"),
            user_text=text, chat_id=recipient, sender=sender, is_group=is_group,
        )
        system += (
            "\nContinue only the pending data-file save. The only permitted tool is write_file, "
            "with path exactly " + selected.filename + ". Do not choose another destination or action. "
            "Use the original request and same-chat context for the content."
        )
        request = selected.request + "\n\nFilename selected for this pending save: " + selected.filename
        reply = get_response(
            system, get_history(recipient, 20), request,
            use_tools=False, allowed_tools=["write_file"],
            on_tool_call=attempts.append, sender=sender, originating_chat_id=recipient,
            tool_argument_guard=_file_continuation.argument_guard(selected),
        )
        if not attempts:
            reply = _file_continuation.NO_WRITE
        elif not reply:
            reply = "The file write was attempted, but its result is not verified. Check the file before trying again."
    try:
        sent = send_message(recipient, reply, **({"is_group": True} if is_group else {}))
    except Exception as exc:
        _file_continuation.observe(sender, recipient, "")
        logger.warning("File continuation reply send uncertain (%s); not retrying", type(exc).__name__)
        return True
    if not sent:
        _file_continuation.observe(sender, recipient, "")
    try:
        save_turn(recipient, "user", f"{sender}: {redact_secret(text)}" if is_group else redact_secret(text))
        if sent:
            save_turn(recipient, "assistant", reply)
    except Exception as exc:
        logger.warning("File continuation history unavailable (%s)", type(exc).__name__)
    return True


def _handle_identity_status(sender: str, text: str, recipient: str, *, is_group: bool = False, has_image: bool = False) -> bool:
    """Describe the verified actor after existing gates, without model rewriting."""
    if has_image:
        return False
    reply = _identity_status.reply(text, sender)
    if reply is None:
        return False
    _send_native_command_reply(sender, text, reply, recipient=recipient, is_group=is_group, known_read_only=True)
    return True


def _handle_acute_safety(sender: str, text: str, recipient: str, *, is_group: bool = False) -> bool:
    """Reply once after access/private-send gates, before persona shortcuts.

    Candidate detection is not evidence of danger. Only supplied same-chat user
    turns and the current message can ground the deterministic response.
    """
    if not _acute_safety.needs_context(text):
        return False
    try:
        history = get_history(recipient, _acute_safety.HISTORY_LIMIT)
    except sqlite3.Error:
        logger.warning("Acute safety history unavailable; checking current message only")
        history = []
    reply = _acute_safety.acute_safety_reply(text, history, sender=sender, is_group=is_group)
    if reply is None:
        return False
    sent = send_message(recipient, reply, **({"is_group": True} if is_group else {}))
    try:
        content = f"{sender}: {redact_secret(text)}" if is_group else redact_secret(text)
        save_turn(recipient, "user", content)
        if sent:
            save_turn(recipient, "assistant", reply)
    except Exception as exc:
        # Do not retry the send or fall through to persona/model replies after
        # an uncertain history write. Delivery and history are separate facts.
        logger.warning("Acute safety reply history unavailable (%s)", type(exc).__name__)
    return True
...
def _handle_admin_dm(sender: str, text: str, image_path: str | None = None) -> None:
    ...
    # A credential never changes the authenticated sender's role. Pending
    # private-send confirmation above remains the separate password-gated flow.
    if text and check_admin_password(text):
        clean = strip_password(text)
        if check_admin_password(clean) or redact_secret(clean) != clean:
            send_message(sender, "I couldn't safely remove that credential. Resend the task without a password. Your admin access is unchanged.")
            return
        if not clean:
            send_message(sender, "A password doesn't change your admin access. What would you like me to do?")
            return
        text = clean
    ...
    if self_status_reply is not None:
        _send_native_command_reply(sender, text, self_status_reply, known_read_only=True)
        return
    model_status_reply = _handle_model_status_question(sender, text)
    if model_status_reply is not None:
        _send_native_command_reply(sender, text, model_status_reply, known_read_only=True)
        return
    length_reply = _non_owner_length_rejection(sender, text)
    if length_reply is not None:
        send_message(sender, length_reply)
        return
    banter_reply = None if (_acute_safety.needs_context(text) or _identity_status.query_kind(text)) else _viral_banter_reply(text, has_image=bool(image))
    ...
    if private_reply is not None:
        ...
        return
    if _handle_acute_safety(sender, text, sender):
        if image:
            _image_buffer.pop(key, None)
        return
    if _handle_identity_status(sender, text, sender, has_image=bool(image)):
        return
    from .research_cron import schedule_from_text as _schedule_research_from_text
    research, questions = _capture_cron_reply(_schedule_research_from_text, sender, text, sender, db_path=BOT_DB_PATH)
    if research is not None:
        _send_cron_reply(sender, text, sender, research.text, questions, record_history=False)
        return
    ...
    if cron_describe_reply is not None:
        ...
        return
    sports_cron_reply, questions = _capture_cron_reply(_sports_recap_cron_from_text, sender, text, originating_chat_id=sender)
    if sports_cron_reply is not None:
        _send_cron_reply(sender, text, sender, sports_cron_reply, questions, record_history=False)
        return
    cron_schedule_reply, questions = _capture_cron_reply(_schedule_cron_from_text, sender, text, originating_chat_id=sender)
    if cron_schedule_reply is not None:
        _send_cron_reply(sender, text, sender, cron_schedule_reply, questions, record_history=False)
        return
    ...
    if cmd_reply is not None:
        _send_native_command_reply(sender, text, cmd_reply)
        return
    ...
    if _handle_image_caption(sender, text, image, sender):
        return
    system = build_system_prompt(persona=None, user_text=text, chat_id=sender, sender=sender, is_group=False)
    if skip_search:
        system += "\n" + _NO_WEB_INFORMATION_INSTRUCTION
    ...
    sent = send_message(sender, reply)
    save_turn(sender, "user", redact_secret(text if not image else f"[image] {text}"))
    if sent:
...
def _handle_friend_dm(sender: str, text: str, image_path: str | None = None) -> None:
    ...
    if cmd == "help":
        ...
        _send_native_command_reply(sender, text, _cmd_help(sender))
        return
    if cmd in ("capability", "capabilities"):
        from .commands import _cmd_capabilities
        _send_native_command_reply(sender, text, _cmd_capabilities(sender))
        return
    if cmd == "mypermissions":
        from .commands import _cmd_mypermissions
        _send_native_command_reply(sender, text, _cmd_mypermissions(sender))
        return
    ...
    if not skip_search and is_ufc_fight_card_request(text):
        ...
        _send_native_command_reply(sender, text, reply)
        return
    if _handle_image_caption(sender, text, image, sender):
        return
    system = (
...
def handle_dm(sender: str, text: str, image_path: str | None = None, trace: _MessageTrace | None = None) -> None:
    from . import image_conversation, reminder_edits

    # Invalidate only this actor/chat's ephemeral edit before native early returns.
    reminder_edits.discard_unrelated(sender, sender, "" if image_path else text,
                                    authorized=is_owner(sender))

    _file_continuation.observe(sender, sender, text, has_image=bool(image_path), authorized=is_owner(sender))
    if not is_owner(sender):
        if is_admin(sender):
            if trace:
    ...
    pending_reply = handle_private_send_confirmation(sender, text or "", allow_password=True)
    if pending_reply is not None:
        _file_continuation.observe(sender, sender, "")
        reminder_edits.clear_drafts(sender)
        send_message(sender, pending_reply)
        return
    private_reply = handle_private_send_request(sender, text, originating_chat_id=sender)
    if private_reply is not None:
    ...
    if _handle_identity_status(sender, text, sender, has_image=bool(image)):
        return
    _clear_superseded_cron_drafts(sender, text, sender, has_image=bool(image))
    if _handle_file_continuation(sender, text, sender, has_image=bool(image)):
        return

    edit_reply = None if image else handle_reminder_edit(sender, text)
    if edit_reply is not None:
        try:
            sent = send_message(sender, edit_reply)
        except Exception:
            reminder_edits.clear_drafts(sender)
            logger.warning("Reminder edit reply could not be sent; not retrying.")
            return
        if not sent:
            reminder_edits.clear_drafts(sender)
        try:
            ...
            logger.warning("Reminder edit history could not be saved; not resending.")
        return
    ...
    research, questions = _capture_cron_reply(_schedule_research_from_text, sender, text, sender, db_path=BOT_DB_PATH)
    if research is not None:
        _send_cron_reply(sender, text, sender, research.text, questions)
        return
    cron_describe_reply = _describe_cron_from_text(sender, text, originating_chat_id=sender)
    if cron_describe_reply is not None:
        send_message(sender, cron_describe_reply)
    ...
    sports_cron_reply, questions = _capture_cron_reply(_sports_recap_cron_from_text, sender, text, originating_chat_id=sender)
    if sports_cron_reply is not None:
        _send_cron_reply(sender, text, sender, sports_cron_reply, questions)
        return

    cron_schedule_reply, questions = _capture_cron_reply(_schedule_cron_from_text, sender, text, originating_chat_id=sender)
    if cron_schedule_reply is not None:
        _send_cron_reply(sender, text, sender, cron_schedule_reply, questions)
        return
    ...
    if fact:
        key, value = fact
        saved = store_user_fact(key, value, source="dm")
        reply = (
            f"Got it — noted that {key}: {value}." if saved else
            "I couldn't confirm that fact was saved. I won't retry it automatically."
        )
        try:
            sent = send_message(sender, reply)
        except Exception as exc:
            sent = False
            logger.warning("Fact acknowledgment send uncertain (%s); not retrying", type(exc).__name__)
        try:
            save_turn(sender, "user", redact_secret(text))
            if sent:
                save_turn(sender, "assistant", reply)
        except Exception as exc:
            logger.warning("Fact acknowledgment history unavailable (%s); not resending", type(exc).__name__)
        return
    ...
    system = _trace_call(
        trace, "prompt_build", prompt_builder,
        persona=active_persona, user_text=text, chat_id=sender, sender=sender, is_group=False,
    )
    if skip_search:
        system += "\n" + _NO_WEB_INFORMATION_INSTRUCTION
    ...
    reply = _trace_call(
        ...
        on_tool_call=file_attempts.append,
    )
    if reminder_intent == "schedule":
    ...
    if not reply:
        ...
        if refusal:
            ...
            # Model refusal prose cannot establish a missing product feature.
            # Preserve its specific limitation without inventing or logging one.
    if reply:
    ...
    _file_continuation.remember(sender, sender, text, reply, delivered=sent,
                                attempted_tools=file_attempts, has_image=bool(image))
    if not sent:
        if trace:
...
def _is_group_cron_draft_followup(sender: str, chat_id: str, text: str) -> bool:
    from . import cron_creation, research_cron

    legacy = cron_creation.is_pending_followup(sender, chat_id, text)
    research = is_owner(sender) and research_cron.is_pending_followup(sender, chat_id, text)
    if not legacy and not research:
        return False
    if not is_owner(sender) and not (
        is_admin(sender)
        and cron_creation.is_pending_followup(sender, chat_id, text, required_action="sports_recap")
    ):
        return False
    return is_gc_enabled(chat_id) and is_owner_in_chat(chat_id, OWNER_ID)


def handle_group_message(sender: str, chat_id: str, text: str, msg: dict | None = None, trace: _MessageTrace | None = None) -> None:
    from . import image_conversation, research_cron

    _file_continuation.observe(sender, chat_id, strip_mention(text), has_image=bool(msg and msg.get("image_path")), authorized=is_owner(sender))
    if not is_owner_in_chat(chat_id, OWNER_ID):
    ...
    if not is_at_mentioned(text):
        ...
        if image_path or not _is_group_cron_draft_followup(sender, chat_id, text):
    ...
    research_followup = (
        is_owner(sender) and is_gc_enabled(chat_id)
        and research_cron.is_pending_followup(sender, chat_id, text)
    )

    if (is_owner(sender) or is_gc_enabled(chat_id)) and not research_followup:
        cmd_reply = handle_group_command(sender, chat_id, command_text)
        if cmd_reply is not None:
            _send_native_command_reply(sender, strip_mention(command_text), cmd_reply, recipient=chat_id, is_group=True)
            return
    ...
    if self_status_reply is not None:
        _send_native_command_reply(sender, clean_text, self_status_reply, recipient=chat_id, is_group=True, known_read_only=True)
        return
    model_status_reply = _handle_model_status_question(sender, clean_text)
    if model_status_reply is not None:
        _send_native_command_reply(sender, clean_text, model_status_reply, recipient=chat_id, is_group=True, known_read_only=True)
        return
    ...
    from .research_cron import schedule_from_text as _schedule_research_from_text
    # Complete requests keep their original route. A retained-draft answer
    # and an image-free identity question pass the private-send checks below.
    research, questions = (None, []) if (
        research_followup or (not image and _identity_status.query_kind(clean_text))
    ) else _capture_cron_reply(_schedule_research_from_text, sender, clean_text, chat_id, db_path=BOT_DB_PATH)
    if research is not None:
        _send_cron_reply(sender, clean_text, chat_id, research.text, questions, is_group=True)
        return

    market_reply = None if image else _market_fast_reply(sender, clean_text, originating_chat_id=chat_id, **({"skip_web_search": True} if skip_search else {}))
    if market_reply is not None:
    ...
    banter_reply = None if (_acute_safety.needs_context(clean_text) or _identity_status.query_kind(clean_text)) else _viral_banter_reply(clean_text, has_image=bool(image))
    if banter_reply is not None:
        if image:
            _image_buffer.pop(key, None)
    ...
    if not skip_search and is_ufc_fight_card_request(clean_text):
        ...
        if not is_admin(sender):
            log_tool_use(sender, UFC_FIGHT_CARD_TOOL)
        reply = get_ufc_fight_card()
        _send_native_command_reply(sender, clean_text, reply, recipient=chat_id, is_group=True)
        return

    if is_admin(sender):
        pending_reply = handle_private_send_confirmation(sender, clean_text, allow_password=False)
        if pending_reply is not None:
            _file_continuation.observe(sender, chat_id, "")
    ...
    if research_followup:
        research, questions = _capture_cron_reply(_schedule_research_from_text, sender, clean_text, chat_id, db_path=BOT_DB_PATH)
        if research is not None:
            _send_cron_reply(sender, clean_text, chat_id, research.text, questions, is_group=True)
            return

    if _handle_acute_safety(sender, clean_text, chat_id, is_group=True):
        if image:
            _image_buffer.pop(key, None)
        return

    if _handle_identity_status(sender, clean_text, chat_id, is_group=True, has_image=bool(image)):
        return
    _clear_superseded_cron_drafts(sender, clean_text, chat_id, is_group=True, has_image=bool(image))
    if _handle_file_continuation(sender, clean_text, chat_id, is_group=True, has_image=bool(image)):
        return
    ...
    sports_cron_reply, questions = _capture_cron_reply(_sports_recap_cron_from_text, sender, clean_text, originating_chat_id=chat_id)
    if sports_cron_reply is not None:
        _send_cron_reply(sender, clean_text, chat_id, sports_cron_reply, questions, is_group=True)
        return

    cron_schedule_reply, questions = _capture_cron_reply(_schedule_cron_from_text, sender, clean_text, originating_chat_id=chat_id)
    if cron_schedule_reply is not None:
        _send_cron_reply(sender, clean_text, chat_id, cron_schedule_reply, questions, is_group=True)
        return
    ...
    if reminder_intent == "cancel":
        reminder_text, _ = _strip_no_search(strip_mention(text, preserve_body=True))
        cancel_reply = _handle_deterministic_reminder_cancel(reminder_text, chat_id)
        if cancel_reply is not None:
    ...
    if reminder_intent == "schedule":
        reminder_text, _ = _strip_no_search(strip_mention(text, preserve_body=True))
        reminder_reply = _handle_deterministic_reminder_schedule(reminder_text, chat_id)
        if reminder_reply is not None:
    ...
    system = _trace_call(
        trace, "prompt_build", prompt_builder,
        persona=active_persona, user_text=clean_text, chat_id=chat_id, sender=sender, is_group=True,
    )
    if skip_search:
        system += "\n" + _NO_WEB_INFORMATION_INSTRUCTION
    ...
    # Unmentioned discussion is quoted background only for a current chat-only
    # reference. It never participates in command, tool, or permission routing.
    if not image and not owner_tool_path and not _should_use_limited_web_tools(clean_text, skip_search):
        try:
            from . import group_context
            excerpts = group_context.quoted_history(
                chat_id, clean_text,
                sender_allowed=lambda participant: is_owner(participant) or is_approved_user(participant),
            )
            if excerpts:
                # Supplemental background must not displace newer directed
                # conversation when provider history retains a bounded suffix.
                # This is priority ordering, not a chronological merge.
                history = [*excerpts, *history]
                has_group_context = True
                if trace:
                    trace.flag("quoted_group_context")
        except Exception as exc:
            logger.warning("Group discussion context unavailable (%s)", type(exc).__name__)
    if trace:
        trace.prompt_chars = len(system or "")
        trace.history_turns = len(history or [])
    ...
    if is_owner(sender):
        _file_continuation.remember(sender, chat_id, clean_text, reply, delivered=sent,
                                    attempted_tools=file_attempts, has_image=bool(image))
    if not sent:
        if trace:
            trace.flag("send_failed")
    ...
    if is_owner(sender):
        try:
            if plain_chat or has_group_context:
                if trace:
                    trace.flag("memory_extract_skipped")
            else:
...
def _remember_unmentioned_group_text(sender: str, chat_id: str, text: str) -> None:
    """Collect bounded background without changing the unmentioned-message return."""
    try:
        from . import group_context
        from .commands import OWNER_COMMANDS
        if not group_context.accepts_text(text) or _is_injection_attempt(text):
            return
        if not is_gc_enabled(chat_id) or not is_owner_in_chat(chat_id, OWNER_ID):
            return
        if not (is_owner(sender) or is_approved_user(sender)):
            return
        if check_admin_password(text) or redact_secret(text) != text or redact_private_send_text_for_log(text) != text:
            return
        if _recovered_message_requires_confirmation({"text": text}):
            return
        lower = text.strip().lower().lstrip("/!")
        if any(lower == command or lower.startswith(command + " ") for command in OWNER_COMMANDS):
            return
        group_context.remember(chat_id, sender, text)
    except Exception as exc:
        logger.warning("Group discussion context skipped (%s)", type(exc).__name__)
...
def handle_message(msg: dict) -> None:
    ...
    if is_group and not is_at_mentioned(text):
        if image_path:
            _buffer_unmentioned_group_image(sender, chat_id, image_path)
            return
        if not _is_group_cron_draft_followup(sender, chat_id, text):
            _remember_unmentioned_group_text(sender, chat_id, text)
            return

    if not check_rate_limit(sender):
        recipient = chat_id if is_group else sender
        trace.set_route("rate_limited")
        if not _rate_limit_notice.allow(sender, recipient):
            return
        _trace_call(
            trace,
            "send",
...
def _record_delivered_cron_post(job_id: int, recipient: str, text: str) -> None:
    """Retain delivered bot-authored context without reopening a completed send."""
    try:
        from .text_safety import normalize_bot_text
        save_turn(recipient, "assistant", normalize_bot_text(text or ""))
    except Exception as exc:
        # History failure must not change delivery state or repeat an accepted send.
        logger.warning("Cron job #%d delivered but history save failed (%s)", job_id, type(exc).__name__)


@schedule_locked
def _check_cron_jobs() -> None:
    """Check native cron occurrences and the separate one-shot self-test."""
    ...
    try:
        ...
        for job_id, expr, action_type, payload_json, last_run in rows:
            if action_type == "research_report":
                # Public research runs in a bounded worker, never in the poll loop.
                from .research_reports import dispatch as _dispatch_research_report
                _dispatch_research_report(BOT_DB_PATH, job_id, now_pst, send_message, on_submitted=_record_delivered_cron_post)
                continue
            try:
            ...
            if (expr or "").strip() == "TEST_2MIN":
                ...
                try:
                    ...
                    conn.close()
                if delivered_post is not None:
                    _record_delivered_cron_post(job_id, *delivered_post)
                ...
                continue
            # Only legacy daily/weekly actions use this occurrence ledger.
            # Research reports retain their separate dispatcher and run claims.
            from .cron_occurrences import LEGACY_ACTIONS, run_due
            if action_type in LEGACY_ACTIONS:
                run_due(
                    BOT_DB_PATH, job_id, datetime.now(timezone.utc), send_message,
                    _record_delivered_cron_post, owner_id=OWNER_ID,
                    clock=lambda: datetime.now(timezone.utc),
                )
            else:
                logger.error("Cron job #%d has unsupported action type", job_id)
