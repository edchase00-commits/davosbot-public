import json
import re
from . import american_odds as _american_odds
import sqlite3
import subprocess
import logging
import os
...
def _is_persona_catalog_request(text: str) -> bool:
    """Recognize read-only persona questions without treating switches as queries."""
    text = re.sub(r"\s+", " ", (text or "").strip().lower()).rstrip(".!?")
    text = re.sub(r"^(?:please|pls)\s+|\s+(?:please|pls)$", "", text)
    return bool(re.fullmatch(
        r"(?:persona(?: (?:list|options|status))?|personas|personalities"
        r"|(?:list|show)(?: me)?(?: (?:your|the|all|available|other))* (?:personas|personalities)"
        r"|(?:what|which)(?: (?:other|available|different))* (?:personas|personalities)"
        r"(?: (?:do|can) (?:you|u) (?:have|use|offer)| are (?:available|there))?"
        r"|what are (?:your|the)(?: (?:other|available|different))* (?:personas|personalities)"
        r"|what(?:'s| is) your (?:current )?(?:persona|personality)"
        r"|(?:what|which) persona are (?:you|u)(?: (?:using|running))?)",
        text,
    ))
...
def _parse_cancel_cron_id(text: str) -> int | None:
    m = re.fullmatch(
        r"(?:cancel|delete|remove|kill|stop|disable)\s+"
        r"(?:#|id\s*#?|(?:cron|job|daily\s+job)\s+(?:id\s*#?|#)?)(\d+)"
        r"(?:\s+please)?\s*[.!?]*",
        text.strip(),
        re.IGNORECASE,
    )
    cron_id = int(m.group(1)) if m else 0
    return cron_id if cron_id > 0 else None
...
def _format_group_tell(target: str, message: str, chat_id: str = "", sender: str = "") -> str:
    ...
    try:
        ...
        system = build_system_prompt(
            persona=persona, user_text=message, chat_id=chat_id, sender=sender, is_group=True,
        )
        prompt = (
...
def handle_command(sender: str, text: str, *, skip_web_search: bool = False) -> str | None:
    ...
    from .dashboard_links import is_dashboard_link_request, is_fantasy_command
    if is_billing_status_request(stripped):
        stripped = "billing"
    elif is_dashboard_link_request(stripped):
        stripped = "fantasy"
    cmd = stripped.lower()
    ...
    # Fantasy roster persistence (deterministic, owner-only)
    from . import roster as _roster
    roster_reply = _roster.handle_roster_message(sender, stripped)
    ...
    if cmd != "personalities" and _is_persona_catalog_request(stripped) and is_owner(sender):
    ...
    # A topic prefix is not an access command, including for non-owner admins.
    if cmd.startswith("fantasy ") and not is_fantasy_command(stripped):
        return None
...
def _cmd_capabilities(sender: str = "") -> str:
    ...
    lines = [
        ...
        "- Recurring jobs: the owner can create quotes, sports recaps, bot health checks, or weekly public research/waiver reports. Admins can create sports-recap crons in the current chat. Ask `can I create a cron?` for your verified access; `cron help` explains setup.",
        ...
        "- Learn by asking: `who am I?`, `what permissions do I have?`, `how do you work?`, or a follow-up about this capability list. These questions do not create jobs, grant access, or install tools.",
        "- Learning limits: saved skills are trigger/reply templates, not new code or model training. New capabilities need a reviewed implementation and deployment before I can claim they work.",
...
def _cmd_cron_help() -> str:
    return "\n".join([
        "Cron setup and controls (all times Pacific):",
        "- the owner: ask `create a new cron`, then choose quotes, sports recap, bot health, or weekly public research/waivers. Admins can create sports-recap crons; other types remain the owner-only.",
        "- Ask from the intended DM or enabled group. In a group, start with @Davos. A new job stays in that chat, not a different recipient named in the text.",
        "- I ask for missing details one at a time. The same sender can answer in that chat within 5 minutes; a group draft answer does not need another mention. A bare yes never creates a cron. Restarting discards unfinished setup.",
        "- Examples for the owner: `create a quote cron at 8am` or `create a weekly research report about battery technology every Friday at 8am`. Waiver reports also need the FAAB budget.",
        "- Quotes, sports, and health: daily or one weekday at one time. Research: one weekly time, with optional start/end dates. Unsupported actions or ambiguous schedules need clarification, not a substitute job.",
        "- the owner: `list crons` shows this chat. Use `list all crons` privately for the cross-chat board; `list group crons` and `list crons just to me` filter it. Check the saved #id, time and destination before repeating a request.",
        "- the owner/admins: `describe cron #7` explains a job; admins can inspect only their current chat. A saved schedule is not proof a report was generated or delivered.",
        "- the owner edits/disables: `set #7 to 6:30am`, `change cron #7 to friday 8pm`, or `cancel cron #7`. Use the actual ID from the list. `cancel cron draft` discards unfinished setup, not a saved job.",
        "- Sports recap example for the owner/admins: `create daily sports recap cron at 6pm`. Ask `can I create a cron?` to check your verified role without creating anything.",
    ])
...
def _cmd_help(sender: str = "") -> str:
    ...
    return (
        ...
        "  Owner-only; password text does not elevate access\n"
        "\n"
        ...
        "- skills — list registered skills + status\n"
        "- skill enable [name] / skill disable [name]\n"
        "- Saved skills are trigger/reply templates, not generated code or new tool handlers\n"
        "\n"
        ...
        "\n"
        "PASSWORD GATE\n"
        "- Passwords confirm an existing eligible private-send request in DM\n"
        "- Password text never changes your verified owner/admin/friend role\n"
        "- Owner-only commands and tools remain owner-only\n"
        "\n"
        f"Personas: {personas}"
    )
...
def _parse_bet_input(text: str) -> dict | str:
    ...
    # An amount is risk unless the user explicitly labels it target profit.
    # "Lakers -110 2u", "Knicks ML +150 1.5u", "Patriots -3.5 spread 2u"
    m = re.match(
        r"^(.+?)\s+([+-]\d{1,7})\s+(?:(to\s+win)\s*)?"
        r"([+-]?(?:\d+(?:\.\d+)?|\.\d+))\s*(?:u(?:nits?)?)?(?:\s+(.*))?$",
        raw, re.IGNORECASE
    )
    if not m:
    ...
    odds = int(m.group(2))
    try:
        amount = _american_odds.positive_amount(m.group(4))
        # Validate odds on either syntax; a zero price cannot be settled.
        target_stake = _american_odds.stake_for_profit(odds, amount)
        stake = float(target_stake if m.group(3) else amount)
        _american_odds.positive_amount(stake)
...
def handle_group_command(sender: str, chat_id: str, text: str) -> str | None:
    ...
    from .dashboard_links import is_dashboard_link_request, is_fantasy_command
    if is_dashboard_link_request(group_command):
        group_command = "fantasy"

    if is_fantasy_command(group_command, is_group=True):
    ...
    if _is_persona_catalog_request(group_command) and (is_owner(sender) or is_approved_user(sender)):
        return _persona_status(chat_id)

    if re.fullmatch(r"(?:crons?|jobs)\s+help[.!?]*", group_command, re.IGNORECASE) and is_admin(sender):
        return _cmd_cron_help()
    ...
    if re.fullmatch(r"help[.!?]*", group_command, re.IGNORECASE):
        return _cmd_help(sender)

    if re.fullmatch(r"capabilit(?:y|ies)[.!?]*", group_command, re.IGNORECASE):
        return _cmd_capabilities(sender)
    ...
    # Only complete controls change group state. Idioms such as "on a scale"
    # and "off the top of your head" are conversation, not enable/disable.
    if re.fullmatch(r"@davos\s+on[.!]?", lower, re.IGNORECASE):
        enable_gc(chat_id)
    ...
    if re.fullmatch(r"@davos\s+off[.!]?", lower, re.IGNORECASE):
        disable_gc(chat_id)
        return "Going quiet."
