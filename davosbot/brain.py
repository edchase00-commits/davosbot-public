...
from .db import connect_bot_db, run_migration, init_legacy_cron_schema
...
from . import action_grounding as _action_grounding
...
_OLLAMA_TRUNCATED_SENTINEL = "__ollama_truncated_reply__"
_OLLAMA_CONTEXT_LIMIT_SENTINEL = "__ollama_context_limit__"


_HELP_INTENT_RE = re.compile(
    r"(?:please\s+)?(?:"
    r"help|capabilit(?:y|ies)"
    r"|what\s+(?:can|do)\s+(?:you|davos|davosbot)\s+do(?:\s+for\s+me)?"
    r"|what\s+capabilities(?:\s+do\s+you\s+have)?"
    r"|what\s+are\s+(?:(?:my|your|the)\s+)?commands"
    r"|how\s+do\s+(?:i|you)\s+use\s+(?:you|this(?:\s+bot)?|davos|davosbot)"
    r")(?:\s+please)?[.!?]*",
    re.IGNORECASE,
)
...
def store_user_fact(key: str, value: str, source: str = "self") -> bool:
    """Confirm a committed fact by exact-row readback; never retry uncertainty."""
    try:
        with connect_bot_db(BOT_DB_PATH) as conn:
            cursor = conn.execute(
        ...
        # Reopen only after the transaction commits. A failed readback does
        # not prove the insert failed, and must not cause another insert.
        with connect_bot_db(BOT_DB_PATH) as conn:
            row = conn.execute(
                "SELECT key, value, source FROM user_facts WHERE id = ?",
                (fact_id,),
            ).fetchone()
        return row == (key, value, source)
    except Exception as exc:
        logger.warning("store_user_fact unconfirmed (%s)", type(exc).__name__)
        return False


def detect_help_intent(text: str) -> bool:
    """True if the message is clearly asking for a command list or capability overview."""
    return bool(_HELP_INTENT_RE.fullmatch((text or "").strip()))


def detect_capability_gap(response: str) -> bool:
    """Detect model refusal language, not proof that a feature is missing."""
...
def _ollama_soft_miss_reason(user_msg: str, reply: str | None) -> str | None:
    if not reply:
        return None
    if reply == _OLLAMA_TRUNCATED_SENTINEL:
        return "output_limit"
    if reply == _OLLAMA_CONTEXT_LIMIT_SENTINEL:
...
def _init_db_tables() -> None:
    ...
    init_legacy_cron_schema(BOT_DB_PATH, run_migration)
...
if not ADMIN_PASSWORD and not os.getenv("DAVOSBOT_SUPPRESS_CONFIG_WARNINGS"):
    import sys as _sys
    print(
        "WARNING: ADMIN_PASSWORD not set in .env - password-confirmed private sends are disabled. "
        "Password text never changes sender permissions.",
        file=_sys.stderr,
    )
...
def detect_reminder_edit_intent(text: str) -> bool:
    """Recognize a direct edit request, not quoted or explanatory mentions."""
    from .reminder_edits import is_edit_request
    return is_edit_request(text)


def handle_reminder_edit(sender: str, text: str) -> str | None:
    """Reschedule in place only after a scoped, validated time and readback."""
    from .reminder_edits import handle_edit
    return handle_edit(sender, text, originating_chat_id=sender, db_path=BOT_DB_PATH)
...
_MAX_OLLAMA_HISTORY_CHARS = 8000
_MAX_OLLAMA_HISTORY_TURN_CHARS = 1600
_OLLAMA_DEFAULT_CONTEXT_TOKENS = 4096
_OLLAMA_REPLY_RESERVE_TOKENS = 512
_OLLAMA_CONTEXT_SAFETY_TOKENS = 128
_OLLAMA_MESSAGE_OVERHEAD_TOKENS = 16
...
def _clip_text_for_model(text: object, max_chars: int) -> str:
    ...
    head = (keep + 1) // 2
    return raw[:head] + marker + raw[-(keep - head):]


def _estimate_ollama_tokens(text: str) -> int:
    """Approximate English at three chars/token; reserve bytes for non-ASCII.

    This is a budget heuristic, not the selected model's tokenizer. Dense code,
    unusual text, and model-specific chat templates can still cost more tokens.
    """
    ascii_chars = sum(character.isascii() for character in text)
    non_ascii_bytes = len(text.encode("utf-8", errors="replace")) - ascii_chars
    return (ascii_chars + 2) // 3 + non_ascii_bytes
...
def _fit_history_for_model(
    ...
    *,
    ...
    """Keep recent turns chronologically, preserving both ends of long turns."""
    if max_chars <= 0 or max_turn_chars <= 0:
        return []
    fitted: list[dict] = []
    remaining = max_chars
    remaining_tokens = max_estimated_tokens
    for turn in reversed(history or []):
        ...
        if not raw:
            continue
        if fitted and remaining <= 200 and len(raw) > remaining:
            break
        turn_budget = min(max_turn_chars, remaining)
        content = _clip_text_for_model(raw, turn_budget)
        if remaining_tokens is not None:
            available_tokens = remaining_tokens - _OLLAMA_MESSAGE_OVERHEAD_TOKENS
            if available_tokens <= 0:
                break
            if _estimate_ollama_tokens(content) > available_tokens:
                low, high = 0, min(len(raw), turn_budget)
                while low < high:
                    middle = (low + high + 1) // 2
                    candidate = _clip_text_for_model(raw, middle)
                    if _estimate_ollama_tokens(candidate) <= available_tokens:
                        low = middle
                    else:
                        high = middle - 1
                content = _clip_text_for_model(raw, low)
            if not content:
                break
            remaining_tokens -= _estimate_ollama_tokens(content) + _OLLAMA_MESSAGE_OVERHEAD_TOKENS
...
def _ollama_history_budget(system: str, user_msg: str, num_predict: int | None = None) -> int | None:
    """Return a local history char ceiling; None means fixed input cannot fit.

    Call with the already compacted system prompt. Unknown provider context
    defaults get a conservative 4096-token assumption without changing options.
    An uncapped output keeps a planning reserve, not a new generation limit.
    """
    context = OLLAMA_NUM_CTX if OLLAMA_NUM_CTX > 0 else _OLLAMA_DEFAULT_CONTEXT_TOKENS
    reply_tokens = num_predict if num_predict and num_predict > 0 else _OLLAMA_REPLY_RESERVE_TOKENS
    remaining = (
        context - reply_tokens - _OLLAMA_CONTEXT_SAFETY_TOKENS
        - 2 * _OLLAMA_MESSAGE_OVERHEAD_TOKENS
        - _estimate_ollama_tokens(system) - _estimate_ollama_tokens(user_msg)
    )
    if remaining < 0:
        return None
    return min(_MAX_OLLAMA_HISTORY_CHARS, remaining * 3)


def _fit_ollama_history(
    system: str, history: list[dict], user_msg: str, num_predict: int | None = None,
) -> list[dict] | None:
    budget = _ollama_history_budget(system, user_msg, num_predict)
    if budget is None:
        return None
    return _fit_history_for_model(
        history, max_chars=budget, max_turn_chars=_MAX_OLLAMA_HISTORY_TURN_CHARS,
        max_estimated_tokens=budget // 3,
    )
...
def _fit_system_for_ollama(system: str) -> str:
    context, raw = _request_context.split_request_context(str(system or ""))
    max_chars = max(0, _MAX_OLLAMA_SYSTEM_CHARS - len(context))
    if len(raw) <= max_chars:
        return context + raw
    ...
    if rules or relevant or facts:
        ...
        overflow = max(0, total_budget - max_chars)
        ...
        fitted = _clip_middle_for_local_prompt(raw, max_chars, "system prompt")

    if len(fitted) > max_chars:
        fitted = _clip_middle_for_local_prompt(fitted, max_chars, "system prompt")
...
def _call_ollama(
    ...
    model: str | None = None,
    truncated_fallback: str | None = None,
    context_fallback: str | None = None,
) -> str | None:
    """Return final text; allow callers to distinguish truncation from outages."""
    ...
    history = _fit_ollama_history(system, history, user_msg, num_predict)
    if history is None:
        logger.info("Ollama input exceeds estimated context budget model=%s", model_name)
        return context_fallback if context_fallback is not None else empty_fallback
    messages = []
    if system:
    ...
    try:
        ...
        if data.get("done_reason") == "length":
            logger.warning(
                "Ollama chat hit its output limit after %.2fs model=%s output_chars=%d",
                elapsed, model_name, len(content),
            )
            _log_bot_event(
                "ollama_truncated_reply",
                {"model": model_name, "num_predict": num_predict, "output_chars": len(content)},
            )
            # A nonempty prefix is still incomplete. Let the caller's existing
            # fallback handle the miss instead of sending a cut-off answer.
            return truncated_fallback if truncated_fallback is not None else empty_fallback
...
def _call_gemini_agentic(
    ...
    tool_argument_guard: object = None,
) -> str | None:
    ...
    tool_argument_guard: optional trusted runtime constraint(name, args).
    """
    ...
    system = _request_context.with_request_context(system, sender, (tool["name"] for tool in tool_defs), originating_chat_id)
    if not tool_defs:
        return _action_grounding.ground_unexecuted_reply(
            user_msg, _call_gemini(system, history, user_msg, image_path=image_path),
        )
    ...
    trace = ToolTrace(user_msg=user_msg)
                ...
                for part in function_calls:
                    ...
                    if name not in available_tool_names:
                        ...
                        if previous is not None:
                            ...
                            allowed_arguments = True
                            if tool_argument_guard is not None:
                                try:
                                    allowed_arguments = tool_argument_guard(name, args) is True
                                except Exception as exc:
                                    logger.warning("Tool argument constraint failed (%s)", type(exc).__name__)
                                    allowed_arguments = False
                            try:
                                if not allowed_arguments:
                                    outcome = ToolOutcome("denied", "The tool arguments exceed this request's scope.",
                                                          "runtime_argument_constraint", error="arguments_not_allowed")
                                else:
                                    if on_tool_call:
                                        on_tool_call(name)
...
def get_response(
    ...
    # Direct providers and every plain fallback have no tools. The agentic call
    # replaces this prefix after resolving its actual function declarations.
    system = _request_context.with_request_context(system, sender, (), originating_chat_id)

    def plain_reply(value):
        return _action_grounding.ground_unexecuted_reply(user_msg, value)

    constrained = {"tool_argument_guard": tool_argument_guard} if tool_argument_guard is not None else {}

    if image_path:
        if use_tools:
            reply = _call_gemini_agentic(system, history, user_msg, image_path=image_path, on_tool_call=on_tool_call, sender=sender, originating_chat_id=originating_chat_id, **constrained)
        elif allowed_tools:
            reply = _call_gemini_agentic(system, history, user_msg, image_path=image_path, allowed_tools=allowed_tools, on_tool_call=on_tool_call, sender=sender, originating_chat_id=originating_chat_id, **constrained)
        else:
            reply = plain_reply(_call_gemini(system, history, user_msg, image_path=image_path))
    ...
    if use_tools:
        reply = _call_gemini_agentic(system, history, user_msg, on_tool_call=on_tool_call, sender=sender, originating_chat_id=originating_chat_id, **constrained)
        reply = _humanize_transient_error(reply)
        reply = _maybe_sass_large_prompt(reply)
        if reply:
            return reply
    ...
    if allowed_tools:
        reply = _call_gemini_agentic(system, history, user_msg, allowed_tools=allowed_tools, on_tool_call=on_tool_call, sender=sender, originating_chat_id=originating_chat_id, **constrained)
        reply = _humanize_transient_error(reply)
        reply = _maybe_sass_large_prompt(reply)
        if reply:
            return reply
    ...
    if _ollama_down:
        ...
        if local_chat_available:
            reply = _call_ollama(
                ...
                truncated_fallback=_OLLAMA_TRUNCATED_SENTINEL,
                context_fallback=_OLLAMA_CONTEXT_LIMIT_SENTINEL,
            )
