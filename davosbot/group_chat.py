...
def strip_mention(text: str, *, preserve_body: bool = False) -> str:
    stripped = (text or "").strip()
    if not stripped:
        return ""

    if preserve_body:
        # Native reminder parsing must see punctuation and newlines exactly:
        # trimming a trailing '-' turns an invalid range into a valid selector.
        # Use the existing mention recognizers without broadening access.
        at_mention = _AT_DAVOS_RE.search(stripped)
        if at_mention:
            return (stripped[:at_mention.start()] + stripped[at_mention.end():]).lstrip(" \t\r\n,:;-").rstrip()
        leading = _plain_leading_mention(stripped)
        if leading:
            return (leading.group("rest") or "").strip()
        trailing = _plain_trailing_mention(stripped)
        if trailing:
            return (trailing.group("body") or "").strip()
        return stripped
