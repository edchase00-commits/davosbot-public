...
from .config import GEMINI_API_KEY, GEMINI_REWRITE_MODEL, TAVILY_API_KEY, BOT_DB_PATH, GENERATED_DIR, DB_PATH, PROJECT_ROOT, normalize_handle
...
TOOL_DEFINITIONS = [
    ...
    {
        "name": "generate_file",
        "description": "Generate a CSV or text file and send it only to the current originating iMessage chat. This cannot send files to another person or chat.",
        "parameters": {
            "type": "object",
            "properties": {
...
def _execute_tool(name: str, args: dict, sender: str = "", originating_chat_id: str = "") -> str | ToolOutcome:
    ...
    try:
        if name == "web_search":
            ...
            destination = _generated_file_destination(args, sender, originating_chat_id)
            if isinstance(destination, ToolOutcome):
                return destination
...
def _canonical_file_destination(value: object) -> tuple[str, bool] | None:
    """Validate runtime/legacy routing values without resolving contact names."""
    ...
    if re.fullmatch(r"\+?[0-9\s().-]+", value) and re.fullmatch(r"\+[1-9][0-9]{7,14}", normalized):
        return normalized, False
    return None


def _generated_file_destination(args: dict, sender: str, originating_chat_id: str) -> tuple[str, bool] | ToolOutcome:
    """Only trusted chat context can select the file's outbound destination."""
    destination = _canonical_file_destination(originating_chat_id)
    if destination is None or (not destination[1] and destination != _canonical_file_destination(sender)):
        return ToolOutcome(
            "denied", "No valid current chat context. Nothing was generated or sent.",
            "originating_chat", error="invalid_file_origin",
        )
    # Older callers may still supply these fields. A mismatch is denied before
    # file creation, not silently redirected into a different chat.
    if (
        not isinstance(args, dict)
        or ("recipient" in args and _canonical_file_destination(args["recipient"]) != destination)
        or ("is_group" in args and (type(args["is_group"]) is not bool or args["is_group"] != destination[1]))
    ):
        return ToolOutcome(
            "denied", "Generated files can only be sent to the current chat. Nothing was generated or sent.",
            "originating_chat", error="file_destination_mismatch",
        )
    return destination
...
def _parse_cron_schedule_command(text: str) -> dict | None:
    ...
    if cron_creation.is_schedule_discussion(raw):
        return None
    lower = cron_creation.control_text(raw).lower()
    if re.search(
...
def _schedule_cron_from_text(sender: str, text: str, originating_chat_id: str = "") -> str | None:
    ...
    owner = is_owner(sender)
    sports_followup = is_admin(sender) and cron_creation.is_pending_followup(
        sender, originating_chat_id, text, required_action="sports_recap",
    )
    if not owner and not sports_followup:
    ...
    parsed = cron_creation.prepare_creation(
        sender, originating_chat_id, text, parsed, _normalize_hhmm,
        required_action="" if owner else "sports_recap",
    )
...
def _parse_cron_edit_command(text: str) -> dict | None:
    ...
    if not re.search(
        ...
        cron_creation.control_text(raw),
        re.IGNORECASE,
    ):
        return None
...
def _edit_cron(
    ...
    try:
        ...
        if old_action == "research_report":
            return f"Research cron #{rid} has its own topic and date window. Cancel it with `cancel cron #{rid}`, then resend the complete research schedule from its target chat. I haven't changed it."
        try:
...
def _parse_cron_cancel_command(text: str) -> dict | None:
    """Parse one complete affirmative command, never selectors from reported text."""
    raw = re.sub(r"^@davos\b[:,]?\s*", "", (text or "").strip(), flags=re.IGNORECASE)
    command = re.fullmatch(
        r"(?:please\s+)?(?:(?:can|could|would|will)\s+you\s+(?:please\s+)?)?"
        r"(?:cancel|delete|remove|kill|stop|disable|turn\s+off)\s+"
        r"(?P<target>.+?)(?:\s+please)?\s*[.!?]*",
        raw, re.IGNORECASE,
    )
    if not command:
        return None
    ...
    id_match = re.fullmatch(
        r"(?:the\s+)?(?:(?:cron|job|daily\s+job)\s+(?:id\s*)?#?\s*(\d+)|(?:id\s*#?|#)\s*(\d+))",
        target, re.IGNORECASE,
    )
    if id_match:
        cron_id = int(id_match.group(1) or id_match.group(2))
        return {"cron_id": cron_id, "time_pt": "", "action": ""} if cron_id > 0 else None

    # Only supported job names and one explicit time are selectors. Unknown
    # words, quoted IDs, retractions and compound requests cannot be discarded.
    names = (
        r"crons?|cron\s+jobs?|daily(?:\s+(?:cron|job))?|"
        r"(?:recurring|scheduled)\s+(?:job|message)|"
        r"morning\s+(?:job|message|quote|one)|"
        r"(?:daily|inspirational|motivational)\s+quote(?:\s+(?:cron|job))?|"
        r"quote\s+(?:job|one|cron)|sports?\s+(?:recap|one)(?:\s+(?:cron|job))?|"
        r"drift\s+(?:check|one)(?:\s+(?:cron|job))?|(?:bot\s+)?health\s+report\s+(?:cron|job)"
    )
    clock_time = (
        r"(?:\d{1,2}:\d{2}(?:\s*(?:a\.?m\.?|p\.?m\.?))?|"
        r"\d{1,2}\s*(?:a\.?m\.?|p\.?m\.?)|noon|midnight)(?:\s+(?:PT|PST|PDT))?"
    )
    named = re.fullmatch(
        rf"(?:the\s+)?(?:(?P<before>{clock_time})\s+)?(?P<name>{names})"
        rf"(?:\s+(?:at\s+)?(?P<after>{clock_time}))?",
        target, re.IGNORECASE,
    )
    if not named or (named.group("before") and named.group("after")):
        return None
    time_text = named.group("before") or named.group("after")
    time_pt = _parse_time_from_text("at " + time_text) if time_text else ""
    if time_text and not time_pt:
        return None
    lower = named.group("name").lower()
    action = ""
    if re.search(r"\b(?:drift|maintenance|health\s+report)\b", lower):
        action = "drift_check"
    elif re.search(r"\bsports?\s+recap\b", lower):
...
def _cancel_cron_from_text(sender: str, text: str, originating_chat_id: str = "") -> str | None:
    ...
    if re.search(
        r"\b(?:cancel|delete|remove|kill|stop|disable|turn\s+off)\s+(?:the\s+)?"
        r"(?:new\s+crons?|cron\s+drafts?|draft\s+crons?|unfinished\s+cron(?:\s+draft)?)\b",
        text or "", re.IGNORECASE,
    ):
        return (
            "That refers to a cron draft, not a saved job. "
            "To discard the unfinished draft, reply `cancel cron draft`. No saved cron was changed."
        )
...
def _sports_recap_request_kind(text: str) -> str:
    from . import cron_creation
    if cron_creation.is_schedule_discussion(text):
        return ""
    lower = cron_creation.control_text(text).lower()
    ...
    if re.search(r"\b(?:create|new|set\s+up|setup|add|make|start|schedule)\b", lower):
        return "create"
    if cron_creation.is_creation_request(text):
        return "create"
    return ""
...
def _sports_recap_cron_from_text(sender: str, text: str, originating_chat_id: str = "") -> str | None:
    ...
    if requested.get("ambiguous_hour"):
        if kind != "edit":
            return cron_creation.prepare_creation(
                sender, originating_chat_id, text, None, _normalize_hhmm, required_action="sports_recap",
            )
...
def _format_cron_description(
    ...
    elif action == "research_report":
        from .research_cron import describe
        try:
            behavior = describe(expr, payload) + " Reads recent public sources at run time; delivery status verifies iMessage submission only."
        except (ValueError, TypeError, KeyError):
            behavior = "Invalid research report configuration; ask the owner to recreate it."
    else:
...
def _list_crons(
    ...
    for rid, expr, action, p in matching:
        ...
        if action == "research_report":
            from .research_cron import describe
            try:
                intro_note = " | " + describe(expr, p)
            except (ValueError, TypeError, KeyError):
                intro_note = " | invalid research report configuration"
        recipient = p.get("recipient", "")
