...
from .config import PROJECT_ROOT, SOUL_PATH, MEMORY_PATH, normalize_handle
from .permissions import is_owner
from .style_directives import format_style_directives_for_prompt
from .roast_intent import is_roast_request
...
def _personality_text_for_prompt(text: str, *, owner_dm: bool = False) -> str:
    cleaned = _sanitize_memory_for_prompt(text)
    if not owner_dm:
        # _cmd_enrichsoul appends private user_facts under this known heading.
        # Strip the whole section, including child headings, from shared views.
        cleaned = re.sub(
            r"^##[ \t]+Known about the owner[ \t]*\r?$.*?(?=^#{1,2}[ \t]+|\Z)",
            "", cleaned, flags=re.IGNORECASE | re.MULTILINE | re.DOTALL,
        ).strip()
    return cleaned


def _light_personality_block(persona_text: str | None, *, owner_dm: bool = False) -> tuple[str, str]:
    """Return the default or active personality text for fast local chat."""
    if persona_text:
        label = "Active Persona"
        block = _personality_text_for_prompt(persona_text, owner_dm=owner_dm).strip()
    else:
        label = "Default Personality"
        block = _personality_text_for_prompt(load_soul(), owner_dm=owner_dm).strip()
...
def _is_owner_dm_context(sender: str | None, chat_id: str | None, is_group: bool | None) -> bool:
    """Allow private facts only for an explicitly identified owner DM.

    These fields must come from the message handler, never model/user text.
    Missing context fails closed, including legacy prompt callers.
    """
    if is_group is not False or not sender or not chat_id:
        return False
    return is_owner(sender) and normalize_handle(chat_id) == normalize_handle(sender)
...
def build_system_prompt(
    ...
    *,
    ...
    owner_dm = _is_owner_dm_context(sender, chat_id, is_group)
    memory = _sanitize_memory_for_prompt(load_memory()) if owner_dm else ""
    persona_text = load_persona(persona) if persona else None

    # Persona fully replaces SOUL.md when active
    base = _personality_text_for_prompt(persona_text if persona_text else load_soul(), owner_dm=owner_dm)
    parts = [base]
...
def build_light_chat_system_prompt(
    persona: str | None = None,
    user_text: str = "",
    chat_id: str | None = None,
    *,
    ...
    date grounding, and owner-DM message-relevant facts so simple Gemma turns
    do not pay the latency cost of the full task/tool prompt.
    """
    owner_dm = _is_owner_dm_context(sender, chat_id, is_group)
    persona_text = load_persona(persona) if persona else None
    personality_label, personality_block = _light_personality_block(persona_text, owner_dm=owner_dm)
    parts = [
