...
class CronCreationPermissionTests(unittest.TestCase):
    ...
    def route(self, text, group=False, *, send_result=True, history=None):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(main, "save_turn", history if history is not None else Mock()))
            stack.enter_context(patch.object(main, "get_response", side_effect=AssertionError("Cron intake should not use LLM")))
            send = stack.enter_context(patch.object(main, "send_message", return_value=send_result))
            if group:
