...
class OpenAIImageTests(unittest.TestCase):
    ...
    def test_food_roast_phrase_does_not_force_image_roast_prompt(self):
        for prompt in (
            "roast chicken", "how long should I roast salmon?",
            "roast frozen broccoli at 400 degrees", "cook a whole fish",
            "please don\u2019t roast me, just explain the mistake",
            "no jokes, cook my friend dinner", "don't make fun of me, explain this",
            ...
            with self.subTest(prompt=prompt):
                intent = openai_images.parse_openai_image_intent(f"image scan {prompt}", has_image=True)
                self.assertEqual("scan", intent.kind)
                self.assertEqual(prompt, intent.prompt)

    def test_literal_phrase_elsewhere_does_not_cancel_requested_image_roast(self):
        for prompt in (
            "roast my friend because he cannot cook chicken",
            "roast my friend's attempt to cook salmon",
            "roast my friend for forgetting to drag the file into the folder",
        ):
            with self.subTest(prompt=prompt):
                intent = openai_images.parse_openai_image_intent(prompt, has_image=True)
                self.assertIn("Roast this image in 1-3 short lines", intent.prompt)
