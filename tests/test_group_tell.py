...
def _load_command_functions(*names):
    names = (*names, "_is_persona_catalog_request")
...
class GroupTellTests(unittest.TestCase):
        ...
        return patch.dict(
            sys.modules,
            {
                ...
                "davosbot.personality": types.SimpleNamespace(
                    build_system_prompt=lambda persona=None, user_text="", **context: f"persona={persona}; user={user_text}; group={context['is_group']}"
                ),
            },
        )
    ...
    def test_group_tell_uses_active_persona_without_tools_or_private_send_claim(self):
        ...
        self.assertIn("group=True", self.calls[0]["system"])
    ...
    def test_group_tell_has_safe_fallback_when_styling_fails(self):
        ...
        with patch.dict(
            sys.modules,
            {
                ...
                "davosbot.personality": types.SimpleNamespace(
                    build_system_prompt=lambda **kwargs: "unused"
                ),
            },
        ):
    ...
    def test_group_tell_does_not_retry_prompt_without_privacy_context(self):
        builder = Mock(side_effect=[TypeError("context rejected"), "synthetic private facts"])
        model = Mock(return_value="unused")
        with patch.dict(sys.modules, {
            "davosbot.brain": types.SimpleNamespace(get_response=model),
            "davosbot.personality": types.SimpleNamespace(build_system_prompt=builder),
        }):
            reply = self.format_group_tell("Chapman", "hello", chat_id="group-chat-guid", sender="+15550000001")

        self.assertEqual("Chapman, they said hello. Don't shoot the messenger.", reply)
        builder.assert_called_once()
        self.assertTrue(builder.call_args.kwargs["is_group"])
