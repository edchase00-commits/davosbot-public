...
from davosbot import failure_copy, permissions
...
def _load_prompt_helpers():
    ...
    namespace = {
        ...
        "_request_context": SimpleNamespace(
            with_request_context=lambda system, *_args: system,
            split_request_context=lambda system: ("", system),
        ),
    }
...
class PromptSizeGuardTests(unittest.TestCase):
    ...
    def test_formatted_owner_intake_logs_once_and_other_handles_cannot_log(self):
        helpers = _load_prompt_helpers()
        prompt = "Review this routing design. " + "x" * helpers["_MAX_USER_MODEL_CHARS"]
        with tempfile.TemporaryDirectory() as tmp:
            helpers["BOT_DB_PATH"] = str(Path(tmp) / "intake.sqlite")
            with closing(sqlite3.connect(helpers["BOT_DB_PATH"])) as conn:
                conn.execute("CREATE TABLE change_log (id INTEGER PRIMARY KEY, request TEXT, reason TEXT)")
            for sender in ("+15550000002", "(555) 000-0002", "555000001", "", None):
                with self.subTest(sender=sender):
                    reply = helpers["get_response"]("", [], prompt, sender=sender)
                    self.assertIn(reply, helpers["_LARGE_PROMPT_SASS"])
            with patch.object(permissions, "OWNER_ID", ""):
                reply = helpers["get_response"]("", [], prompt, sender="(555) 000-0001")
                self.assertIn(reply, helpers["_LARGE_PROMPT_SASS"])
            reply = helpers["get_response"]("", [], prompt, sender=" (555) 000-0001 ")
            with closing(sqlite3.connect(helpers["BOT_DB_PATH"])) as conn:
                rows = conn.execute("SELECT request, reason FROM change_log").fetchall()
        self.assertIn("Logged oversized Codex intake #1", reply)
        self.assertEqual(1, len(rows))
        self.assertIn("status=review_only", rows[0][1])
