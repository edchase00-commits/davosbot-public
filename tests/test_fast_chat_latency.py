    ...
    def test_owner_dm_trivial_casual_chat_uses_fast_table(self):
        ...
        for prompt in prompts:
            with self.subTest(prompt=prompt):
                sent = []
                saved = []
                # Ambiguous acknowledgements may be an immediate safety
                # follow-up. That one bounded read must still avoid the model.
                history = Mock(return_value=[]) if main._acute_safety.needs_context(prompt) else Mock(
                    side_effect=AssertionError("history should not load for unrelated trivial chat")
                )
                ...
                patches = [
                    ...
                    patch.object(main, "get_history", history),
                ...
                if main._acute_safety.needs_context(prompt):
                    history.assert_called_once_with("+15550000001", main._acute_safety.HISTORY_LIMIT)
                else:
    ...
    def test_owner_dm_active_personas_bare_chat_uses_light_persona_prompt(self):
        for persona in ("ATL", "Gruden"):
            with self.subTest(persona=persona):
                ...
                def fake_prompt_builder(*, persona=None, user_text="", chat_id=None, sender=None, is_group=None):
                    prompt_calls.append({"persona": persona, "user_text": user_text, "chat_id": chat_id, "sender": sender, "is_group": is_group})
                ...
                self.assertEqual([{"persona": persona, "user_text": "Yo", "chat_id": "+15550000001", "sender": "+15550000001", "is_group": False}], prompt_calls)
                self.assertEqual([("+15550000001", 2)], history_calls)
