...
from davosbot import roast_intent, simple_chat
...
def _load_roast_helpers():
    ...
    namespace = {"re": re, "_simple_chat": simple_chat, "_roast_intent": roast_intent}
...
class RoastRoutingTests(unittest.TestCase):
    ...
    def test_food_roasting_does_not_trigger_banter_mode(self):
        helpers = _load_roast_helpers()
        for prompt in (
            "how long should I roast chicken", "how long should I roast salmon?",
            "roast frozen broccoli at 400 degrees", "cook a whole fish",
            "please don\u2019t roast me, just explain the mistake",
            "don't make fun of me, explain this", "drag the file into this folder",
            "cook my friend dinner", "cook us a meal",
            "I don't want jokes, cook my friend", "I don't need any jokes, roast my friend",
            "I don\u2019t want any jokes, roast my friend",
        ):
            with self.subTest(prompt=prompt):
                self.assertFalse(helpers["_is_roast_request"](prompt))
                self.assertFalse(helpers["_should_keep_roast_chat_only"](prompt))

    def test_literal_phrase_does_not_override_separate_explicit_roast(self):
        helpers = _load_roast_helpers()
        prompt = "roast my friend because he cannot cook chicken"
        self.assertTrue(helpers["_is_roast_request"](prompt))

    def test_negated_roast_does_not_block_requested_file_tool_route(self):
        helpers = _load_roast_helpers()
        prompt = "don't roast me, read the file and explain the error"
        self.assertFalse(helpers["_should_keep_roast_chat_only"](prompt))
        self.assertTrue(helpers["_should_use_owner_tools"](prompt, False))
