...
from davosbot import action_grounding, failure_copy, permissions, simple_chat
...
def _load_routing_helpers(
    *,
    ...
    wanted_assigns = {
        ...
        "_OLLAMA_TRUNCATED_SENTINEL",
        "_OLLAMA_CONTEXT_LIMIT_SENTINEL",
        "_last_ollama_check",
        "_ollama_down",
    }
    wanted_funcs = {
    ...
    namespace = {
        ...
        "_action_grounding": action_grounding,
        "_request_context": SimpleNamespace(with_request_context=lambda system, *_args: system),
...
class ModelRoutingTests(unittest.TestCase):
    def setUp(self):
        owner = patch.object(permissions, "OWNER_ID", "+15550000001")
        owner.start()
        self.addCleanup(owner.stop)

    def test_formatted_owner_handles_reach_existing_advanced_routes(self):
        for sender in ("+15550000001", "15550000001", "5550000001", " (555) 000-0001 "):
            for prompt, route in (
                ("compare the architecture tradeoffs", "complex_reasoning"),
                ("review this diff for bugs", "code_review"),
            ):
                with self.subTest(sender=sender, route=route):
                    helpers, gemini, ollama, events = _load_routing_helpers()
                    self.assertTrue(permissions.is_owner(sender))
                    reply = helpers["get_response"]("system", [], prompt, sender=sender)
                    self.assertEqual("advanced reply", reply)
                    gemini.assert_called_once()
                    self.assertTrue(gemini.call_args.args[0].endswith("system"))
                    self.assertEqual(([], prompt), gemini.call_args.args[1:])
                    self.assertEqual(
                        {"model": "gemini-3.5-flash", "source": f"{route}_direct"}, gemini.call_args.kwargs,
                    )
                    ollama.assert_not_called()
                    self.assertEqual(route, events[0][1]["route"])

    def test_formatted_nonowners_and_missing_senders_stay_on_local_route(self):
        for sender in ("+15550000002", " (555) 000-0002 ", "555000001", "friend@example.test", "", None):
            with self.subTest(sender=sender):
                helpers, gemini, ollama, events = _load_routing_helpers()
                reply = helpers["get_response"]("", [], "review this diff for bugs", sender=sender)
                self.assertEqual("ollama reply", reply)
                gemini.assert_not_called()
                ollama.assert_called_once()
                self.assertEqual([], events)

    def test_formatted_owner_does_not_enable_an_uncallable_advanced_route(self):
        for route in ("", "disabled", "ollama:gemma4", "codex:gpt-5.4"):
            with self.subTest(route=route):
                helpers, gemini, ollama, events = _load_routing_helpers(complex_route=route, code_route=route)
                reply = helpers["get_response"]("", [], "review this diff for bugs", sender="(555) 000-0001")
                self.assertEqual("ollama reply", reply)
                gemini.assert_not_called()
                ollama.assert_called_once()
                self.assertEqual([], events)

    def test_unconfigured_owner_does_not_select_advanced_route(self):
        helpers, gemini, ollama, events = _load_routing_helpers()
        helpers["OWNER_ID"] = ""
