...
from davosbot import commands, image_conversation, main, market, permissions, personality, tool_outcomes, tools
...
class NoWebRouteTests(unittest.TestCase):
    ...
    def test_actual_handlers_scope_private_memory_before_model_calls(self):
        private_fact = "Synthetic private project codename is lilac."
        with ExitStack() as stack:
            stack.enter_context(patch.object(permissions, "OWNER_ID", OWNER))
            stack.enter_context(patch.object(personality, "load_soul", return_value="Shared voice."))
            stack.enter_context(patch.object(personality, "format_style_directives_for_prompt", return_value=""))
            memory = stack.enter_context(patch.object(personality, "load_memory", return_value=private_fact))
            builders = [stack.enter_context(patch.object(main, name, wraps=getattr(personality, name)))
                        for name in ("build_system_prompt", "build_light_chat_system_prompt")]
            for sender in (OWNER, ADMIN, FRIEND):
                for group in (False, True):
                    for image in (False, True):
                        with self.subTest(sender=sender, group=group, image=image):
                            memory.reset_mock()
                            self.route("Thoughts about the private project?", sender=sender, group=group, image=image)
                            self.model.assert_called_once()
                            prompt = self.model.call_args.args[0]
                            if sender == OWNER and not group:
