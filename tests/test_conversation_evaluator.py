...
class ConversationEvaluatorTests(unittest.TestCase):
    ...
    def test_current_prompt_definitions_load_without_runtime_imports_or_file_reads(self):
        ...
        roast_source = (evaluator.ROOT / "davosbot" / "roast_intent.py").read_text(encoding="utf-8")
        ...
        with patch.object(Path, "read_text", side_effect=AssertionError("unexpected private read")):
            namespace = evaluator.prompt_namespace(source, roast_source=roast_source)
            ...
            self.assertFalse(namespace["_needs_roast_mode"]("roast salmon"))
            self.assertTrue(namespace["_needs_roast_mode"]("roast my friend"))

    def test_roast_helper_uses_supplied_variant_without_importing_it(self):
        source = "from .roast_intent import is_roast_request\ndef _needs_roast_mode(text): return is_roast_request(text)"
        for expected in (False, True):
            helper = f"import forbidden_dependency\ndef is_roast_request(text): return {expected}"
            namespace = evaluator.prompt_namespace(source, roast_source=helper)
            self.assertEqual(expected, namespace["_needs_roast_mode"]("roast salmon"))
    ...
    def test_cli_evaluates_requested_extended_case_on_both_prompts_without_semantic_pass_claim(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch.object(evaluator, "_git_output", side_effect=["a" * 40, source, "b" * 40, roast_source]), patch.object(evaluator.requests, "Session", return_value=self.session):
        ...
        self.assertEqual(report["roast_helper_sha256"]["before"], report["roast_helper_sha256"]["after"])
        self.assertIsNotNone(report["roast_helper_sha256"]["after"])
