...
class BlockedAlertStateTests(unittest.TestCase):
    """auto_deploy_blocked must fire on state changes, not every poll."""

    def setUp(self):
        self.tmp = TemporaryDirectory()
        self.addCleanup(self.tmp.cleanup)
        self.config = _config(repo_root=Path(self.tmp.name))
        env = patch.dict(
            auto_deploy.os.environ,
            {"AUTO_DEPLOY_BLOCKED_REMINDER_SECONDS": "3600"},
            clear=False,
        )
        env.start()
        self.addCleanup(env.stop)

    def _wrapped(self, alerts):
        base = lambda event, message, metadata=None: alerts.append((event, message)) or True  # noqa: E731
        return auto_deploy._stateful_blocked_alert(self.config, base)

    def test_transition_alerts_once_then_suppresses(self):
        alerts = []
        wrapped = self._wrapped(alerts)
        with patch.object(auto_deploy.time, "time", return_value=1000.0):
            self.assertTrue(wrapped("auto_deploy_blocked", "dirty", None))
        self.assertEqual([("auto_deploy_blocked", "dirty")], alerts)
        # Same block ten minutes later: suppressed.
        with patch.object(auto_deploy.time, "time", return_value=1600.0):
            self.assertFalse(wrapped("auto_deploy_blocked", "dirty", None))
    ...
    def test_reminder_fires_after_interval(self):
        ...
        wrapped = self._wrapped(alerts)
        with patch.object(auto_deploy.time, "time", return_value=1000.0):
            wrapped("auto_deploy_blocked", "dirty", None)
        with patch.object(auto_deploy.time, "time", return_value=1000.0 + 3600):
            self.assertTrue(wrapped("auto_deploy_blocked", "dirty", None))
        self.assertEqual(2, len(alerts))
        self.assertEqual("auto_deploy_blocked", alerts[1][0])
        self.assertTrue(alerts[1][1].startswith("still blocked:"))

    def test_reason_change_realerts_immediately(self):
        alerts = []
        wrapped = self._wrapped(alerts)
        with patch.object(auto_deploy.time, "time", return_value=1000.0):
            wrapped("auto_deploy_blocked", "dirty", None)
            self.assertTrue(wrapped("auto_deploy_blocked", "not a fast-forward", None))
        self.assertEqual(2, len(alerts))
        self.assertEqual("not a fast-forward", alerts[1][1])

    def test_state_survives_watcher_restart(self):
        alerts = []
        with patch.object(auto_deploy.time, "time", return_value=1000.0):
            self._wrapped(alerts)("auto_deploy_blocked", "dirty", None)
        # New wrapper instance, as after a watcher restart: no re-alert.
        with patch.object(auto_deploy.time, "time", return_value=1600.0):
            self.assertFalse(self._wrapped(alerts)("auto_deploy_blocked", "dirty", None))
        self.assertEqual(1, len(alerts))
    ...
    def test_other_events_pass_through(self):
        ...
        wrapped = self._wrapped(alerts)
        with patch.object(auto_deploy.time, "time", return_value=1000.0):
            self.assertTrue(wrapped("auto_deploy_failed", "boom", None))
            self.assertTrue(wrapped("auto_deploy_failed", "boom", None))
        self.assertEqual(2, len(alerts))

    def test_recovered_alert_fires_once(self):
        blocked_alerts = []
        recovered_alerts = []
        recovered_base = lambda event, message, metadata=None: recovered_alerts.append((event, message)) or True  # noqa: E731
        with patch.object(auto_deploy.time, "time", return_value=1000.0):
            self._wrapped(blocked_alerts)("auto_deploy_blocked", "dirty", None)
        self.assertTrue(auto_deploy._maybe_alert_recovered(self.config, "up_to_date", recovered_base))
        self.assertEqual([("auto_deploy_recovered", "auto-deploy recovered: dirty")], recovered_alerts)
        # State cleared: a second recovery check stays silent.
        self.assertFalse(auto_deploy._maybe_alert_recovered(self.config, "up_to_date", recovered_base))
        self.assertEqual(1, len(recovered_alerts))

    def test_no_recovered_alert_without_prior_block(self):
        base = Mock(return_value=True)
        self.assertFalse(auto_deploy._maybe_alert_recovered(self.config, "up_to_date", base))
        base.assert_not_called()

    def test_failed_outcome_is_not_recovery(self):
        blocked_alerts = []
        recovered_alerts = []
        recovered_base = lambda event, message, metadata=None: recovered_alerts.append((event, message)) or True  # noqa: E731
        with patch.object(auto_deploy.time, "time", return_value=1000.0):
            self._wrapped(blocked_alerts)("auto_deploy_blocked", "dirty", None)
        self.assertFalse(auto_deploy._maybe_alert_recovered(self.config, "failed", recovered_base))
        self.assertEqual([], recovered_alerts)
        # Block state is preserved: a later up_to_date still reports recovery.
        self.assertTrue(auto_deploy._maybe_alert_recovered(self.config, "up_to_date", recovered_base))
        self.assertEqual(1, len(recovered_alerts))
