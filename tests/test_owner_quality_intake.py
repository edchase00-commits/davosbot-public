...
class OwnerQualityIntakeTests(unittest.TestCase):
    ...
    def test_capability_gap_distinguishes_missing_context_from_refusal(self):
        detect = _load_capability_gap_detector()

        self.assertFalse(detect("I need the spreadsheet before I can analyze that."))
        self.assertFalse(detect("Please upload the file so I can inspect it."))
        self.assertFalse(detect("I don't have enough context to do that."))
